...
def query_db(sql):
    ...
    result = cur.fetchall()[0][0]  # 获取所有查询结果
    logging.debug(result)
    cur.close()  # 关闭游标
    conn.close()  # 关闭连接
    # return len(result)  # 返回结果
    return result

if __name__ == '__main__':
    query_db("select count(1) from enterprise_health where enabled=1")
...
#     conn = get_db_conn()  # 获取连接
#     cur = conn.cursor()  # 建立游标
#     cur.execute("select * from enterprise_health where enabled='{}'".format(enable))  # 执行sql
#     conn.commit()
